 ...
 * always be %NULL.)
 *
 * Return value: the list of usernames, which must be freed with
 * soup_auth_free_saved_users().
 **/
GSList *
 ...
 * soup_auth_free_saved_users:
 * @auth: a #SoupAuth
 * @users: a list of usernames returned from
 * soup_auth_get_saved_users().
 *
 * Frees the list returned from soup_auth_get_saved_users().
 **/
void
soup_auth_free_saved_users (SoupAuth *auth, GSList *users)
{
	g_return_if_fail (SOUP_IS_AUTH (auth));
 ...
 * soup_auth_authenticate_and_save() for that.
 * (soup_auth_has_saved_password() is an internal method, which is
 * used by the code that actually saves and restores the passwords.)
 **/
void
